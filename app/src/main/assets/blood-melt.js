...
(function (global) {
  ...
  const FRAGMENT_SHADER = `
    ...
    void main() {
      ...
      float adhesion = (glyphStick * 0.28 + frameStick * u_edgeAdhesion) * release;
      boundary += adhesion;
  ...
  class BloodMelt {
    constructor(options = {}) {
      ...
      Object.assign(this.canvas.style, {
        ...
        pointerEvents: 'none', zIndex: String(this.zIndex), background: 'transparent'
      });
    ...
    _initGl() {
      ...
      this.adhesionTexture = gl.createTexture();
      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE_2D, this.adhesionTexture);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
      gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
      if (this.adhesionCanvas) {
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, this.adhesionCanvas);
      } else {
    ...
    _frame(time) {
      ...
      this.draw(progress);
      if (progress < 1) return requestAnimationFrame(this._frame);
      this.running = false;
    ...
    destroy() {
      ...
      if (this.adhesionTexture) this.gl.deleteTexture(this.adhesionTexture);
      if (this.buffer) this.gl.deleteBuffer(this.buffer);
      if (this.program) this.gl.deleteProgram(this.program);
      this.adhesionCanvas = null;
      this.canvas.remove();
    }
  }
